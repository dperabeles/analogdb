...
### 2026-05-13: Approved Editorial Paper Shell Pass

Completed:

- User confirmed the Next.js preview still did not look close enough to the GitHub Pages beta.
- Re-read `analog-db-dashboard.html` as the visual source of truth instead of continuing from the previous dark Next.js shell.
- Added an approved-only `approved-shell` class to private Next.js routes.
- Kept public/login/access states on the dark access shell.
- Moved approved desktop routes toward the GitHub Pages editorial shell:
  - light paper canvas `#f5ede0`
  - paper ink `#1a1510`
  - warm paper cards `#fbf5e8`
  - dark editorial sidebar `#2a2620`
  - beta-style `#8a3a22` accent
  - Fraunces editorial serif for page titles, section numerals, and roll stock names
  - Inter for labels and supporting copy
  - DM Mono for archive ids and compact controls
- Updated private route shells for:
  - `/dashboard`
  - `/stats`
  - `/timeline`
  - `/equipment`
  ...
  - `/rolls/[code]`
  - `/rolls/[code]/edit`
- Restyled dashboard/list primitives under `approved-shell` so they read closer to the beta table/editorial archive:
  - paper-colored workspace
  - compact sidebar brand block
  - numbered sidebar nav motif
  - beta-like chevron before roll ids
  - lighter filter controls
  - paper-line table/list borders
  - Fraunces roll stock names
- Added static regression coverage in `tests/next-editorial-shell-parity.test.js`.

Validation commands used:

```bash
node --test tests/next-editorial-shell-parity.test.js tests/next-ui-parity-baseline.test.js tests/next-dashboard-ui-parity.test.js
npm run typecheck
npm run build
node --test auth-recovery.test.js tests/camera-lens-quick-add.test.js tests/camera-quick-mode.test.js tests/next-auth-gates.test.js tests/next-roll-read-flows.test.js tests/next-roll-write-flows.test.js tests/next-admin-flows.test.js tests/next-equipment-flows.test.js tests/next-stats-timeline-flows.test.js tests/next-mobile-navigation.test.js tests/next-ui-parity-baseline.test.js tests/next-dashboard-ui-parity.test.js tests/next-editorial-shell-parity.test.js tests/roll-format-normalization.test.js
python3 -m unittest tests/test_rejected_admin_ui.py tests/test_film_catalog.py tests/test_exposure_settings.py
npx --yes vercel env pull .env.local --environment=preview --git-branch feature/nextjs-vercel-migration --yes
npm run dev -- --hostname 127.0.0.1 --port 3000
curl -sS -I http://127.0.0.1:3000/
...
- New editorial shell parity static test passed.
- Existing UI parity and dashboard parity static tests passed.
- `tsc --noEmit` passed.
- `next build` passed.
...
- First patch accidentally applied `approved-shell` to admin access states. Corrected it so public, pending, and rejected access states remain dark, and only approved/private admin states use the paper shell.
- `next dev` again rewrote `next-env.d.ts`; restored it before committing.
- Pulling Vercel envs created `.env.local`; it was deleted after smoke because it can include a temporary `VERCEL_OIDC_TOKEN`.
- This pass changes the global approved desktop shell, but detailed component parity is still incomplete. Next visual slices should focus on exact dashboard index/workflow layout, roll detail, and equipment cards.
...
- Continue exact parity passes:
  ...
  - roll detail mobile/desktop hero and sections
  - equipment cards and quick-edit controls
  - stats/timeline editorial layouts
